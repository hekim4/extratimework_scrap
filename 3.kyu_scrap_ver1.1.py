...
작성기록 : 2021-03-14 (ver.0)
          2021-03-15 (ver.1 - 서지, 한국어서지 추출코드 추가 및 제작되는 파일에 페이지넘버도 추가)
          2021-03-15 (ver.1.1 - 서지 추출코드 문제 발생부 해결, 속도는 저하될 수 있음)
                                상세서술 : 규장각 홈페이지 구조를 분석해보니 앞부분과 뒷부분 소스가 달라서 
                                           strong과 strong onclick을 둘다 파싱할 수 있는 find("<strong")로 변경함.
                                           정확한 파싱을 위한 idx_bname_mid, idx_korname_mid 탐색과정이 추가됨.
...
with open("kyu_web_scrap_{}({}-{}p).txt".format(time_stamp, pageNum, end_pNum), "a", encoding='UTF-8', newline='') as f:
    ...
    while 1:
        ...
        for i in range(1, 21):
            ...
            id, idx_s, idx_e, idx_bname, idx_korname = tmp_data.find("value"), tmp_data.find("청구기호"), tmp_data.find("편저자"), tmp_data.find("<strong"), tmp_data.find("<span")
            idx_bname_end, idx_korname_end = tmp_data.find("</strong>"), tmp_data.find("</span>")
            idx_bname_mid = tmp_data[idx_bname:idx_bname_end].find(".")
            idx_korname_mid = tmp_data[idx_korname:idx_korname_end].find(">")+1

            if tmp_data[id+7:id+17] == "":
                    break
            else:    
                writer.writerow([tmp_data[id+7:id+17], tmp_data[idx_bname+idx_bname_mid+1:idx_bname_end].strip(), tmp_data[idx_korname+idx_korname_mid+6:idx_korname_end].strip(), tmp_data[idx_s+6:idx_e-15], tmp_data.find("old/btn_img"), tmp_data.find("old/btn_txt"), tmp_data.find("old/btn_toc"), pageNum])
